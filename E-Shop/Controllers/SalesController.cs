        ...
        public ActionResult Buy2(int id)
        {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    db.Sales.Add(satis);
                    db.SaveChanges();
                    ViewBag.islem = "THE PURCHASE WAS SUCCESFUL AND WE WISH YOU A PLEASANT SHOPPING";
                }
            }
            catch (Exception)
            {

                ViewBag.islem = "PURCHASE FAILED";
            }
            return View("islem");
        }
        ...
        public ActionResult Buyall(decimal? Tutar)
        {
            if (User.Identity.IsAuthenticated)
            {
                ...
                if (model!=null)
                {
                    if (kid==null)
                    {
                        ViewBag.tutar = "There are no items in your cart";
                    }
                    else if (kid!=null)
                    {
